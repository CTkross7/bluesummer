...
"""
BLUE SUMMER — Forge 통합 관리 모듈 v5.2
...
# =====================================================================
# Forge processing.py hr_additional_modules NoneType 패치  ← 신규
# =====================================================================

def _patch_processing_py():
    """
    Forge modules/processing.py 의 NoneType 버그를 수정한다.

    문제 (processing.py line 1405):

        if hasattr(self, 'hr_additional_modules') and
               'Use same choices' not in self.hr_additional_modules:

        hr_additional_modules 속성이 존재하지만 값이 None 일 때
        'Use same choices' not in None
            → TypeError: argument of type 'NoneType' is not iterable

    이 버그로 Hires.fix 가 활성화된 모든 fallback(FULL, NO-AD,
    SAFE-HR)이 HTTP 500 으로 실패하며, MINIMAL(Hires 없음)만
    통과하게 된다.

    수정:
        hasattr() 대신 getattr(..., None) 을 사용해
        None 을 falsy 로 처리한다.

    멱등: 이미 패치된 경우 재실행해도 안전하다.
    """
    ...
    try:
        ...
        OLD = (
            "if hasattr(self, 'hr_additional_modules') "
            "and 'Use same choices' not in self.hr_additional_modules:"
        )

        NEW = (
            "if getattr(self, 'hr_additional_modules', None) "
            "and 'Use same choices' not in self.hr_additional_modules:"
        )
        ...
        if NEW in text:
            ...
            L.log(
                "processing.py : hr_additional_modules 패치 이미 적용됨"
            )
        ...
        L.ok(
            "processing.py : hr_additional_modules NoneType 패치 완료"
        )
...
def pre_pin():
    ...
    # -----------------------------------------------------------------
    # processing.py NoneType 패치  ← 신규
    # -----------------------------------------------------------------

    _patch_processing_py()
...
def install_env(
    ...
    # -----------------------------------------------------------------
    # CLIP launcher 검증 + processing.py 패치  ← processing 추가
    # -----------------------------------------------------------------
